...
{
    class FoodSelling : ReportForm
    {
        int numberOfFoods = 0;
        double TotalMoney = 0;
        public FoodSelling(Time kindOfTime, Panel pnlOptionalTime, CrystalReportViewer crystalReportViewer)
                : base(kindOfTime, pnlOptionalTime, crystalReportViewer)
        {
            this.reportTitle = new ReportTitle("Hàng hóa bán ra", reportTime);
            reportPath = Application.StartupPath.Remove(Application.StartupPath.IndexOf("\\bin\\Debug"), 10) + "\\GUI\\Report\\Template\\FoodSelling.rpt";
        }

        public override void loadInfo()
        {
            dataTable = BLL.Report.Ins.getFoodListBetweenDate(reportTime.startTime, reportTime.endTime);
