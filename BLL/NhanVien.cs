...
namespace Restaurant_Management.BLL
{
    class NhanVien
    {
        ...
        public nhanvienDTO getNV(string id)
        {
            ...
            p.Hoten = rs.Rows[0][1].ToString();
            p.Gtinh = rs.Rows[0][2].ToString();
            p.Ngsinh = Convert.ToDateTime(rs.Rows[0][3].ToString());
            p.Dchi = rs.Rows[0][4].ToString();
            p.Cmnd = rs.Rows[0][5].ToString();
            p.Trangthai = (rs.Rows[0][6].ToString() == "True" ? true : false);
            p.Vitri = rs.Rows[0][7].ToString();
            p.ImgStr = rs.Rows[0][8].ToString();
            return p;
      }
