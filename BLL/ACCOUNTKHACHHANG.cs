...
namespace Restaurant_Management.BLL
{
    class ACCOUNTKHACHHANG
    {
        ...
        {
            string query = "SELECT * FROM ACCOUNTKHACHHANG WHERE USERNAME = @userName AND PASSWORDS = @pass ";
            DataTable rs = Provider.Ins.ExcuteQuery(query, new object[] { userName, pass });
            return (rs.Rows.Count > 0);
        }
        public string getID(string userName)
        {
            string query = "SELECT MAKH FROM ACCOUNTKHACHHANG WHERE USERNAME = @userName ";
            DataTable rs = Provider.Ins.ExcuteQuery(query , new object[] { userName });
            if (rs.Rows.Count <= 0) return null;
            string s = rs.Rows[0][0].ToString();
            return s;
        }
